describe('Headless CI route tests', () => {
  it('GET / returns welcome message', () => {
    cy.request('/').then((response) => {
      expect(response.status).to.eq(200);
      expect(response.body.message).to.eq('Welcome to the headless CI demo API');
    });
  });
  ...
  it('GET /api/items returns two items', () => {
    cy.request('/api/items').then((response) => {
      ...
      expect(response.body.items[0].name).to.eq('Monitor CI');
    });
  });
});
