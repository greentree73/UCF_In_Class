...
app.put('/products/:id', async (req, res) => {
	try {
		...
		// Filter by _id using new ObjectId(id)
		// Use $set operator to update price, stock, and add updatedAt field
		...
		// Check if a document was matched (matchedCount)
		// If matchedCount is 0, return 404 status with message 'Product not found'
		if (result.matchedCount === 0) {
			return res.status(404).json({ message: 'Product not found' });
		}
		
		// Send a JSON response with success message and counts
...
app.post('/products/:id/tags', async (req, res) => {
	try {
		...
		// $addToSet adds to array only if value doesn't already exist
		...
		// Check if the product was found (matchedCount === 0)
...
app.delete('/products/:id', async (req, res) => {
	try {
		...
		// Check if a document was deleted (deletedCount === 0)
		// If deletedCount is 0, return 404 with message 'Product not found'
		if (result.deletedCount === 0) {
			return res.status(404).json({ message: 'Product not found' });
		}
		
		// Send JSON response with success message and deletedCount
		res.json({ 
