...
export const QuestionList: React.FC<QuestionListProps> = ({ onEditQuestion }) => {
  ...
  const { 
    ...
    // Apollo maps this object to GraphQL variables by name:
    // { sortBy } -> $sortBy in GetQuestions($sortBy: SortOrder)
    variables: { sortBy },
  ...
  const { 
    ...
    // { query: searchQuery } maps to $query in SearchQuestions($query: String!)
  ...
  const { 
    ...
    // { tag: selectedTag } maps to $tag in GetQuestionsByTag($tag: String!)
